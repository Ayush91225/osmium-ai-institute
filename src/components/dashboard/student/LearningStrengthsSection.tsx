      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="relative">
          <select
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium appearance-none pr-8 cursor-pointer ${isDarkMode ? 'bg-zinc-800 text-white border-zinc-700' : 'bg-gray-100 text-gray-900 border-gray-200'} border`}
          >
            <option value="DSA">DSA</option>
            <option value="Web Dev">Web Dev</option>
            <option value="Database">Database</option>
          </select>
          <svg width="14" height="14" viewBox="0 0 256 256" fill="currentColor" className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${isDarkMode ? 'text-zinc-400' : 'text-gray-600'}`}>
            <path d="M213.66,101.66l-80,80a8,8,0,0,1-11.32,0l-80-80A8,8,0,0,1,53.66,90.34L128,164.69l74.34-74.35a8,8,0,0,1,11.32,11.32Z"/>
          </svg>
        </div>
      </div>

      <div className={`flex gap-0 border-b ${isDarkMode ? 'border-zinc-800' : 'border-gray-200'}`}>
        <button
        ...
        <button
          className={`flex-1 py-2 text-xs font-medium ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`}
        >
          Suggestions
        </button>
      </div>
