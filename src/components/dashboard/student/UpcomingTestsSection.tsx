  ...
  return (
    <div className={`rounded-xl p-5 border ${isDarkMode ? 'bg-zinc-900/50 border-zinc-800/50' : 'bg-white border-gray-200'}`}>
      <div className="flex items-start justify-between mb-1">
        ...
        <button className={`p-1 ${isDarkMode ? 'text-zinc-400 hover:text-white' : 'text-gray-400 hover:text-gray-600'}`} aria-label="More options">
      ...
      <div className="space-y-3 mt-6">
        {tests.map((test, index) => (
          <div key={index} className={`p-4 rounded-lg border`} style={{ backgroundColor: '#FEFEFE', borderColor: isDarkMode ? '#3f3f46' : '#e5e7eb' }}>
            <div className="flex justify-between items-start mb-3">
              ...
              <span className={`text-xs px-3 py-1.5 rounded border bg-white`} style={{ borderStyle: 'dashed', borderColor: '#827151', color: '#827151' }}>
                {test.timeLeft}
              </span>
            </div>
      ...
      <div className="flex items-center justify-center gap-3 mt-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`w-7 h-7 flex items-center justify-center rounded text-xs font-medium ${isDarkMode ? 'bg-zinc-700 text-white' : 'bg-gray-200 text-gray-900'}`}>
            {currentPage}
          </span>
          <span className={`text-xs ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`}>of</span>
          <span className={`text-xs ${isDarkMode ? 'text-zinc-400' : 'text-gray-500'}`}>{totalPages}</span>
        </div>
        <button 
